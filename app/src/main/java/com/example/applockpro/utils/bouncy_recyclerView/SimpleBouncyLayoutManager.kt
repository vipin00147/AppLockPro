    ...
    defStyleRes: Int = 0) : LinearLayoutManager(context, attrs, defStyleAtr, defStyleRes)
{
    ...
    private fun handleScroll(delta: Int, recycler: RecyclerView.Recycler?, state: RecyclerView.State?): Int {
        ...
        if (abs(_overscrollTotal) > 0 &&
            ...
            else if (abs(toScroll) > abs(_overscrollTotal)) {
        ...
        dampen -= abs(_overscrollTotal) / (_maxOverscroll * (1.0 / friction)) // Alter the base dampen with our MaxOverscroll and Tension values
    ...
    private fun updateOverscroll(overscroll: Double) {
        if (abs(overscroll) < Double.MIN_VALUE) {
            return
        }
